...
{
  double sA[M + 1];//= linspace(0, 1, M + 1);
  double sB[M + 1];
  double s[2 * M + 1];
  double t[2 * N + 1];
  double P3_start[3];
  ...
  double P5[3];
  double P6[6];
  double a, h, d, x, y, P5angle, P6angle, asdf;
  double d5, d6;
  int Mm;
  int coonsDebug = 0;

  //We're gonna need some temp variables
  double tmp1[3];
  double tmp1_L;
  double tmp2[3];
  double tmpvec1[3];
  ...
  tmp1[2] = P1[2] - P_ref[2];
  tmp1_L = length(tmp1);
  P4_start[0] = tmp1_L * cos(start_angle);
  P4_start[1] = tmp1_L * sin(start_angle);
  P4_start[2] = 0.0;
  ...
  tmp1[2] = P2[2] - P_ref[2];
  tmp1_L = length(tmp1);
  P3_start[0] = tmp1_L * cos(start_angle);
  P3_start[1] = tmp1_L * sin(start_angle);
  P3_start[2] = 0.0;
  ...
  if (debug){
    ...
    sprintf(msg, "M=%d\n", M);
    ModelicaMessage(msg);
    sprintf(msg, "P4_start={%g, %g, %g}\n", P4_start[0], P4_start[1], P4_start[2]);
    ModelicaMessage(msg);
    sprintf(msg, "P3_start={%g, %g, %g}\n", P3_start[0], P3_start[1], P3_start[2]);
    ModelicaMessage(msg);
  }
  ...
  if (debug){
    ...
    */
    sprintf(msg, "P5={%g, %g, %g}\n", P5[0], P5[1], P5[2]);
    ModelicaMessage(msg);
    sprintf(msg, "P6={%g, %g, %g}\n", P6[0], P6[1], P6[2]);
    ModelicaMessage(msg);
  }
  
  // Find P5 angle
  /*
  tmpvec1[0] = P5[0] - P1[0] - (P5[0] - P1[0]);
  tmpvec1[1] = 0;
  tmpvec1[2] = 0;
  d5 = length(tmpvec1);
  tmpvec2[0] = P6[0] - P2[0] - (P6[0] - P2[0]);
  tmpvec2[1] = 0;
  tmpvec2[2] = 0;
  d6 = length(tmpvec2);
  */
  ...
  tmp1[1] = 0.0;
  tmp1[2] = 0.0;
  tmpvec1[0] = P5[0] - P1[0] - tmp1[0];
  tmpvec1[1] = P5[1] - P1[1] - tmp1[1];
  tmpvec1[2] = P5[2] - P1[2] - tmp1[2];

  tmp1[0] = P6[0] - P2[0];
  tmp1[1] = 0.0;
  tmp1[2] = 0.0;
  tmpvec2[0] = P6[0] - P2[0] - tmp1[0];
  tmpvec2[1] = P6[1] - P2[1] - tmp1[1];
  tmpvec2[2] = P6[2] - P2[2] - tmp1[2];
  ...
  if (debug){
    char msg[BUFFSIZE];
    sprintf(msg, "tmpvec1={%g, %g, %g}\n", tmpvec1[0], tmpvec1[1], tmpvec1[2]);
    ModelicaMessage(msg);
    sprintf(msg, "tmpvec2={%g, %g, %g}\n", tmpvec2[0], tmpvec2[1], tmpvec2[2]);
    ModelicaMessage(msg);
    sprintf(msg, "d5=%g\n", d5);
    ModelicaMessage(msg);
    sprintf(msg, "d6=%g\n", d6);
  ...
  //
  P5[1] = P5[0] * tan(start_angle / 2);
  P6[1] = P6[0] * tan(start_angle / 2);
  P5[2] = -sqrt(d5 * d5 - P5[1] * P5[1]);
  P6[2] = -sqrt(d6 * d6 - P6[1] * P5[1]);
  //
  ...
  tmp1[1] = P_ref[1] + P5[1];
  tmp1[2] = P_ref[2] + P5[2];
  ATv(P5, P_R_T, tmp1, 3, 3);

  tmp2[0] = P_ref[0] + P6[0];
  tmp2[1] = P_ref[1] + P6[1];
  tmp2[2] = P_ref[2] + P6[2];
  ATv(P6, P_R_T, tmp2, 3, 3);
  
  if (debug){
    char msg[BUFFSIZE];
    sprintf(msg, "P_R_T[1,1] = %g\n", P_R_T[0]);
    ModelicaMessage(msg);
    sprintf(msg, "P5new={%g, %g, %g}\n", P5[0], P5[1], P5[2]);
    ModelicaMessage(msg);
    sprintf(msg, "P6new={%g, %g, %g}\n", P6[0], P6[1], P6[2]);
    ModelicaMessage(msg);
  }
  
  // Find approximate locations
  Mm = (M + 1)/2;
  tmp1[0] = P_ref[0] + P4_start[0];
  tmp1[1] = P_ref[1] + P4_start[1];
  tmp1[2] = P_ref[2] + P4_start[2];
  ATv(rot1, P_R_T, tmp1, 3, 3);

  tmp2[0] = P_ref[0] + P3_start[0];
  tmp2[1] = P_ref[1] + P3_start[1];
  tmp2[2] = P_ref[2] + P3_start[2];
  ATv(rot2, P_R_T, tmp2, 3, 3);

  tmp5[0] = P_ref[0] + P1[0];
  tmp5[1] = P_ref[1] + P1[1];
  tmp5[2] = P_ref[2] + P1[2];
  ATv(rot3, P_R_T, tmp5, 3, 3);

  tmp6[0] = P_ref[0] + P2[0];
  tmp6[1] = P_ref[1] + P2[1];
  tmp6[2] = P_ref[2] + P2[2];
  ATv(rot4, P_R_T, tmp6, 3, 3);
  
  Av(tmpvec1, P_R_T, P1, 3, 3);
  Av(tmpvec2, P_R_T, P2, 3, 3);
  tmpvec1[0] += P_ref[0];
  tmpvec1[1] += P_ref[1];
  tmpvec1[2] += P_ref[2];
  tmpvec2[0] += P_ref[0];
  ...
  for(int j=0;j<N;j++){
    for(int i=0;i<Mm-1;i++){            
      CoonsPatch(rot1, rot2, P6, P5, s[2 * i+1], t[2 * j+1], coonsDebug, tmp3);
      CoonsPatch(rot3, rot4, P6, P5, s[2 * i+1], t[2 * j+1], coonsDebug, tmp7);

      for(int ii=0;ii<3;ii++){
	A[N*(M-1-i)*3 + j*3 + ii] = tmp3[ii];
	A[N*3*i + 3*j + ii] = tmp7[ii];
      }
      if (debug){
	char msg[BUFFSIZE];
	sprintf(msg, "A[%d,%d] = {%g, %g, %g}\n",i,j, A[N*3*i + 3*j + 0], A[N*3*i + 3*j + 1], A[N*3*i + 3*j + 2]);
	ModelicaMessage(msg);
	sprintf(msg, "A[%d,%d] = {%g, %g, %g}\n",M-1-i,j, A[N*3*(M-1-i) + 3*j + 0], A[N*3*(M-1-i) + 3*j + 1], A[N*3*(M-1-i) + 3*j + 2]);
	ModelicaMessage(msg);
      }
    }
    CoonsPatch(tmpvec1, tmpvec2, P6, P5, 1.0, t[2 * j+1], coonsDebug, tmp8);
    for(int ii=0;ii<3;ii++){
      A[N*3*(Mm-1) + 3*j + ii] = tmp8[ii]; 
    }
    if (debug){
      char msg[BUFFSIZE];
      sprintf(msg, "A[%d,%d]={%g, %g, %g}\n", Mm-1, j, A[N*3*(Mm-1) + 3*j + 0], A[N*3*(Mm-1) + 3*j + 1], A[N*3*(Mm-1) + 3*j + 2]);
      ModelicaMessage(msg);
    }
  }
}
...
void Cloth_G1_bend_Init(double* P1, double* P2, double* P3, double* P4, int M, int N, int* M_ind, int size_M_ind, int* N_ind, int size_N_ind, double D, int debug, double* k)
{
  ...
  //D := E_mod * th ^ 3 / (12 * (1 - nu ^ 2));
  //for i in 1:size(M_ind, 1) loop
  for(int i=0;i<size_M_ind;i++){
    //for j in 1:size(N_ind, 1) loop
    for(int j=0;j<size_N_ind;j++){
      if (N_ind[j] == 1){
	CoonsPatch(P1, P2, P3, P4, s[M_ind[i]-1], t[N_ind[j]], coonsDebug, tmp1);
	CoonsPatch(P1, P2, P3, P4, s[M_ind[i]-1], t[N_ind[j]-1], coonsDebug, tmp2);
	tmp3[0] = tmp1[0] - tmp2[0];
	tmp3[1] = tmp1[1] - tmp2[1];
	tmp3[2] = tmp1[2] - tmp2[2];	
	width = 0.5*length(tmp3);

      }else if (N_ind[j] == N + 1){
	CoonsPatch(P1, P2, P3, P4, s[M_ind[i]-1], t[N_ind[j]-1], coonsDebug, tmp1);
	CoonsPatch(P1, P2, P3, P4, s[M_ind[i]-1], t[N_ind[j]-2], coonsDebug, tmp2);
	tmp3[0] = tmp1[0] - tmp2[0];
	tmp3[1] = tmp1[1] - tmp2[1];
	tmp3[2] = tmp1[2] - tmp2[2];	
	width = 0.5*length(tmp3);
...
void Cloth_G1_Init(double* P1, double* P2, double* P3, double* P4, int M, int N, double G_xy, double thickness, int debug, double* A)
{
  ...
  for(int j=0;j<N;j++){
    for(int i=0;i<M;i++){
      CoonsPatch(P1, P2, P3, P4, s[i], t[j+1], coonsDebug, tmp1);
      CoonsPatch(P1, P2, P3, P4, s[i], t[j], coonsDebug, tmp2);
      tmp3[0] = tmp1[0] - tmp2[0];
      tmp3[1] = tmp1[1] - tmp2[1];
      tmp3[2] = tmp1[2] - tmp2[2];
      h = length(tmp3);
      CoonsPatch(P1, P2, P3, P4, s[i+1], t[j], coonsDebug, tmp4);
      tmp5[0] = tmp4[0] - tmp2[0];
      tmp5[1] = tmp4[1] - tmp2[1];
      tmp5[2] = tmp4[2] - tmp2[2];
      w = length(tmp5);
      k = G_xy * thickness *(2*(h/w)/(1+((h/w)*(h/w))));
      A[N*i+j] = k;
      if (debug){
	char msg[BUFFSIZE];
	sprintf(msg, "h = %g\n", h);
...
void Cloth_G1s0_Init(double* P1, double* P2, double* P3, double* P4, int M, int N, int debug, double* A)
{
  ...
  for(int j=0;j<N;j++){
    //for i in 1:M loop
    for(int i=0;i<M;i++){
      CoonsPatch(P1, P2, P3, P4, s[i + 1], t[j + 1], coonsDebug, tmp1);
      CoonsPatch(P1, P2, P3, P4, s[i], t[j], coonsDebug, tmp2);
      tmp3[0] = tmp1[0] - tmp2[0];
      tmp3[1] = tmp1[1] - tmp2[1];
      tmp3[2] = tmp1[2] - tmp2[2];
      L = length(tmp3);
      //      length := Vectors.norm(SASLibrary.Math.CoonsPatch(P1, P2, P3, P4, s[i + 1], t[j + 1]) - SASLibrary.Math.CoonsPatch(P1, P2, P3, P4, s[i], t[j]));
      A[N*i+j] = L;  
    }
  }
}
