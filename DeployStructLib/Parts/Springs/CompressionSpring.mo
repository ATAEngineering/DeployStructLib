...
model CompressionSpring "Linear translational compression-only spring with optional mass"
  ...
  input Types.Color relaxedColor = {0, 128, 0} "Color when spring is relaxed" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animation = true", enable = animation));
  input Types.Color compressedColor = {0, 255, 255} "Color when spring is in compression" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animation = true", enable = animation));
  ...
  Translational.CompressionSpring1D spring(c = c, d = d, s_rel0 = s_unstretched, use_events = use_events) annotation(Placement(transformation(extent = {{-8, 40}, {12, 60}}, rotation = 0)));
  Modelica.Mechanics.MultiBody.Visualizers.Advanced.Shape lineShape(shapeType = "cylinder", color = noEvent(if spring.f < 0.0 then compressedColor else relaxedColor), specularCoefficient = specularCoefficient, length = lineForce.length, width = width, height = coilWidth * 2, lengthDirection = lineForce.e_rel_0, widthDirection = Frames.resolve1(frame_a.R, {0, 1, 0}), extra = numberOfWindings, r = frame_a.r_0) if world.enableAnimation and animation;
equation
...
Copyright &copy; 1998-2016, Modelica Association and DLR.
     ...
     <p>
     This spring only exerts force when in compression.
     </p>
     <p>
