...
model CompressionSpring1D
  "Linear 1D translational spring that only exerts force when in compression"
...
equation
  ...
  f_d = noEvent(if compression then d * der(s_rel) else 0.0);
  f = semiLinear(s_rel - s_rel0, 0.0, c) + f_d;
